...
local releaseInfo = HttpService:JSONDecode(game:HttpGetAsync("https://api.github.com/repos/" .. user .. "/zephyrite/releases"))[1]
...
if readFile and writeFile then
    ...
    if not ran or releaseInfo.tag_name ~= result then
        if hasFolderFunctions then
            ...
            createFolder("zephyrite")
            createFolder("zephyrite/user")
            createFolder("zephyrite/user/" .. user)
            createFolder("zephyrite/user/" .. user .. "/methods")
            createFolder("zephyrite/user/" .. user .. "/modules")
            createFolder("zephyrite/user/" .. user .. "/objects")
            createFolder("zephyrite/user/" .. user .. "/ui")
            createFolder("zephyrite/user/" .. user .. "/ui/controls")
            createFolder("zephyrite/user/" .. user .. "/ui/modules")
        end
        ...
        function environment.import(asset)
            ...
            elseif web then
                if readFile and writeFile then
                    local file = (hasFolderFunctions and "zephyrite/user/" .. user .. '/' .. asset .. ".lua") or ("zephyrite-" .. user .. '-' .. asset:gsub('/', '-') .. ".lua")
                    local content

                    if (isFile and not isFile(file)) or not importCache[asset] then
                        content = game:HttpGetAsync("https://raw.githubusercontent.com/" .. user .. "/zephyrite/" .. branch .. '/' .. asset .. ".lua")
                    ...
                    assets = { loadstring(game:HttpGetAsync("https://raw.githubusercontent.com/" .. user .. "/zephyrite/" .. branch .. '/' .. asset .. ".lua"), asset .. '.lua')() }
                end
            else
                assets = { loadstring(readFile("zephyrite/" .. asset .. ".lua"), asset .. '.lua')() }
            end
